...
**Bluetooth Linux** : relancer GerMaCrise (`install.sh` / `demarrer.sh` installent Chromium s’il manque, puis Web Bluetooth). **Bluetooth Windows** : Chrome / Edge natif. Firefox (tous OS) : pas de BLE (USB = Serial). Voir [BIPPER-WEB.md](./BIPPER-WEB.md) § Navigateurs.
...
| USB / Bluetooth indisponible | Passer en HTTPS (ou rester sur `localhost`). BLE : Chrome/Edge (Windows) ou GerMaCrise Linux (`install.sh` installe Chromium s’il manque). Firefox : USB seulement. |
